# 工序识别模块

## 模块目标

根据 `part_feature` 和工序规则生成可解释的 `process_route`。工序识别必须规则主导，AI 只负责解释说明。

## 主责

开发者 A 主责。

## 输入

| 输入 | 说明 |
|---|---|
| `part_feature` | 统一零件特征。 |
| 工序字典 | 标准工序编码、名称、排序。 |
| 工序触发规则 | 材料、几何、孔、精度、表面处理等规则。 |
| 风险规则 | 低置信度、高风险、未知字段。 |

## 输出

| 输出 | 说明 |
|---|---|
| `process_route` | 工序路线和候选结果。 |
| 工序触发原因 | 每个工序的命中依据。 |
| 工序置信度 | 0-1。 |
| 待确认标记 | 是否需要人工确认。 |

## 工序字典

工序识别、工程量计算、价格规则、人工修改和历史样本必须使用同一套工序编码。
第一版先使用代码版统一工序字典，后续再迁移到数据库字典。
当前代码版字典在 `backend/app/process_dictionary.py`，工序路线生成在
`backend/app/process_recognition.py`，报价核心通过该模块生成 `process_route`。

| 工序编码 | 工序名称 | 工序类型 | 说明 |
|---|---|---|---|
| `review_drawing` | 审图/3D确认 | 前处理 | 确认 PDF、STEP、关键尺寸、公差和技术要求。 |
| `material_prepare` | 备料 | 材料 | 按材料和毛坯尺寸备料。 |
| `saw_cut` | 锯切下料 | 下料 | 板料、棒料、块料初步开料。 |
| `wire_cut_blank` | 线割开料 | 下料 | 精密小件或硬料开料。 |
| `surface_grinding_rough` | 平面粗磨 | 磨削 | 建立基准、控制厚度和平面度。 |
| `cnc_milling` | CNC铣削 | 机加工 | 粗铣、精铣、型腔、台阶面加工。 |
| `drilling` | 钻孔 | 孔加工 | 普通通孔、盲孔底孔。 |
| `countersink` | 沉孔/沉头孔 | 孔加工 | 沉孔、沉头孔、倒角孔。 |
| `tapping` | 攻牙 | 孔加工 | 螺纹孔加工。 |
| `precision_hole` | 精孔加工 | 精加工 | 铰孔、镗孔、慢走丝修孔等。 |
| `wire_cut_profile` | 线切割外形 | 线切割 | 异形外轮廓、内孔、窄槽加工。 |
| `heat_treatment` | 热处理 | 热处理 | 淬火、调质、渗氮、真空热处理等。 |
| `straightening` | 校平/校直 | 矫正 | 薄片件、长条件热后矫正。 |
| `finish_grinding` | 精磨 | 磨削 | 热后精磨、厚度控制、平面度控制。 |
| `deburr` | 去毛刺 | 后处理 | 去毛刺、飞边、锐角倒钝。 |
| `pre_plating_cleaning` | 镀前清洗 | 表处前处理 | 除油、清洗、活化。 |
| `chemical_nickel` | 化学镍 | 表面处理 | 化学镀镍。 |
| `post_plating_inspection` | 镀后检验 | 检验 | 镀层外观、膜厚、孔径复检。 |
| `inspection` | 终检 | 检验 | 尺寸、外观、硬度、关键孔检测。 |
| `protective_packaging` | 防划伤包装 | 包装 | 单件隔离、防弯曲、防碰伤包装。 |
| `turning` | 车削 | 机加工 | 轴类件粗车、精车、端面和台阶加工，第一版估算计价并需复核。 |
| `cylindrical_grinding` | 圆磨 | 磨削 | 轴类件外圆、台阶或配合面精磨，第一版估算计价并需复核。 |
| `laser_cut` | 激光切割 | 下料 | 板材或钣金外形切割，第一版估算计价并需复核。 |
| `edm` | 放电加工 | 机加工 | 复杂型腔、窄槽、清角等放电加工，第一版估算计价并需复核。 |
| `unmapped_operation` | 未登记工序 | 人工确认 | 图纸、STEP 或融合特征中识别到，但当前工序字典尚未登记的工序。 |
| `manual_review` | 人工复核 | 人工确认 | 解析、工艺或报价风险需要人工确认。 |

## 触发规则

| 条件 | 工序 |
|---|---|
| 任意支持零件 | 备料、下料、检验、包装。 |
| 板件、块件、异形小件 | CNC 或线切割候选。 |
| 存在普通孔 | 钻孔。 |
| 存在沉孔或沉头孔 | 沉孔。 |
| 存在螺纹孔候选 | 攻牙。 |
| H7、E8、G6、±0.01、高精度孔 | 精孔。 |
| 外轮廓切割、薄片件、板件 | 线切割候选。 |
| 高平面度、精密面、热后修正 | 磨削。 |
| PDF 或技术要求出现热处理 | 热处理。 |
| PDF 或技术要求出现化学镀、镀镍等 | 化学镀。 |
| 技术要求出现去毛刺、防划伤 | 去毛刺。 |
| 轴类件 | 车削；有热处理或精度要求时增加圆磨候选。 |
| 复杂件 | CNC 复杂加工估算；放电加工候选。 |
| AI 基于 PDF、STEP 或融合特征识别到字典外工序 | `unmapped_operation`，保留原始工序名，必须人工确认。 |

## 冲突消解

| 冲突 | 规则 |
|---|---|
| CNC 与线切割都命中 | 按零件类型、外形复杂度、厚度和规则优先级选择主工序，可保留另一工序为待确认候选。 |
| 钻孔与精孔都命中 | 精孔不能替代钻孔，钻孔作为前置，精孔作为精加工。 |
| 热处理前后精加工 | 热处理后需要磨削或精孔时必须排序到热处理之后。 |
| 表面处理与精孔 | 表面处理影响孔径时必须生成风险。 |
| 高复杂度、轴类或特殊工艺 | 不阻断报价，输出首版估算工序并标记需复核。 |
| 字典外工序 | 输出 `unmapped_operation`，不自动计价，必须由人工映射到已有工序、新增字典工序、保留一次性人工报价或删除。 |

## 工序排序

推荐顺序：

```text
备料
-> 下料
-> CNC / 线切割
-> 钻孔 / 沉孔 / 攻牙
-> 热处理
-> 磨削 / 精孔
-> 去毛刺
-> 表面处理
-> 检验
-> 包装
```

## 置信度

| 情况 | 置信度建议 |
|---|---|
| 明确规则命中且证据完整 | 0.85-1.0 |
| 规则命中但证据不完整 | 0.6-0.85 |
| AI 或候选推断 | 0.4-0.7 |
| 冲突未消解 | 低于 0.7 并待确认 |

## 不做什么

- 不计算工程量。
- 不计算价格。
- 不让 AI 将字典外工序作为正式可自动报价工序；AI 只能把明确证据识别到的字典外工序放入 `unmapped_operation` 复核通道。
- 不把解释文本当作规则依据。

## 验收标准

- 必选工序召回率 >= 90%。
- 工序误判率 <= 15%。
- 每个工序有触发原因。
- 每个工序有排序。
- 每个工序有置信度和待确认标记。

## AI Coding 注意事项

- 不要用自然语言判断替代规则表。
- 不要在没有证据时强行输出高置信度。
- 不要把工序顺序写散在多个地方，应集中维护。
- 不要让工序识别依赖页面字段。

